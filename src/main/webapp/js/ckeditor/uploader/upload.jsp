...
<%@ page import="com.dayainfo.ssp.common.util.UploadSuffixVerifyUtil" %>
	...
	if(ServletFileUpload.isMultipartContent(request)){
		...
		for (FileItem item : fileItemsList) {
			if (!item.isFormField()) {
				...
				String pref=fileName.substring(fileName.lastIndexOf(".")+1);
				if(UploadSuffixVerifyUtil.uploadSuffixVerify(pref, UploadSuffixVerifyUtil.formReportSuffix)){
					fileName = "file" + System.currentTimeMillis() + fileName.substring(fileName.lastIndexOf("."));
					//定义文件路径，根据你的文件夹结构，可能需要做修改
					String clientPath = "js/ckeditor/uploader/upload/" + type + fileName;
					...
					out.println("<script type='text/javascript'>alert('不能上传txt, pdf, doc, docx, xls, xlsx, excel, ppt, pptx, wps, bmp, gif, jpg, jpeg, pic, png, tif, rar, zip以外后缀的文件!')</script>");
					break;
				}
			}
		}
